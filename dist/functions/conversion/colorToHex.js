...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx, [code]) {
        try {
            const result = convert_1.ColorConverter.convert(code, typings_1.ColorFormat.hex);
            if (!result) {
                return this.customError(`Could not convert "${code}" to HEX — make sure it is a valid color format.`);
            }
